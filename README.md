...
Provide fully automated `Ubuntu with i3` development environment that is easy to setup and maintain.

### Why Ubuntu?

Currently it's only one approved Linux distro at my work.

### Why Ansible?

Ansible replicates what we would do to setup a development environment pretty well.

## Requirements

### Operating System

This Ansible playbook only supports `Ubuntu` distribution. This is by design to provide a consistent development expierence across hosts.
...
```bash
$ bash -c "$(curl -fsSL https://raw.githubusercontent.com/AlexStasko/dotfiles-ubuntu/main/bin/dotfiles)"
```
